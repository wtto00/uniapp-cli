...
declare global {
  var UniAppConfig: UniConfig
}

export async function getConfig(): Promise<UniConfig> {
  if (!global.UniAppConfig) {
    global.UniAppConfig = (await safeAwait(readJsonFile<UniConfig>('.uniapp.json')))[1] ?? {}
  }
  return global.UniAppConfig
}
