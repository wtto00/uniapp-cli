...
declare global {
  var UniAppManifest: ManifestConfig
}

export async function getManifestJson(): Promise<ManifestConfig> {
  if (!global.UniAppManifest) {
    global.UniAppManifest = await readJsonFile<ManifestConfig>('src/manifest.json')
  }
  return global.UniAppManifest
}
