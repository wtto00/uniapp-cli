...
import {
  type AndroidAbiFilters,
  AndroidDir,
  App,
  Log,
  type ValidArchitectures,
  readJsonFile,
} from '@wtto00/uniapp-common'
import type { UniPlugin } from './files/dcloud_uniplugins.json.js'
import { LibsPath } from './files/libs.js'
import type { Results } from './results.js'

export async function appendPlugins(results: Results) {
  ...
  for (const pluginName in nativePlugins) {
    ...
    const packageJson = await readJsonFile<NativePluginPackage>(packageJsonPath)
